...
SPREADSHEET_ID = "1QWLwJEc1G7qfj3QB_N6EtHUACtYgbNDad-jfyCFq0as"
    ...
    for hour_idx in range(1, len(rows) + 1):
        ...
        for left, right in NEIGHBOUR_PAIRS:
            try:
                ...
                # Dynamic locale-friendly conditional math: Country1 - Country2
                formula = f'=IFERROR(IF({col_L}{row_n}-{col_R}{row_n}<0; 0; {col_L}{row_n}-{col_R}{row_n}); "")'
...
def main() -> None:
    while True:
        ...
        for target_date in target_dates:
            print(f"\n[1/3] Adatok lekérése ({target_date.isoformat()})...")
            try:
            ...
            print(f"[2/3] Google Sheets feltöltés folyamatban...")
            try:
                save_to_google_sheets(target_date, rows)
                print(f"[3/3] SIKER: '{target_date.strftime('%Y.%m.%d.')}' frissítve.")
